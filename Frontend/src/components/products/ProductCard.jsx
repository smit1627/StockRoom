...
export default function ProductCard({ product, actions = false }) {
    ...
    return <article className="product-card">
        <Link className="product-card-image" to={`/products/${product.id}`} aria-label={`View ${product.name}`}>
            {product.imageUrl ? <img src={product.imageUrl} alt={product.name}/> : <span>{product.name?.[0] || "P"}</span>}
            <em className={`stock-badge ${stockClass}`}>{stockLabel}</em>
        </Link>
        <div className="product-card-body">
            <div className="product-card-heading"><div><p className="product-card-category">{product.category?.name || "Uncategorised"}</p><Link to={`/products/${product.id}`}>{product.name || "Unnamed part"}</Link></div>{actions && <Link className="product-card-edit" to={`/products/${product.id}/edit`} aria-label={`Edit ${product.name}`}><i className="bi bi-pencil"/></Link>}</div>
            <p className="product-card-sku">{product.sku ? `SKU · ${product.sku}` : "No SKU assigned"}</p>
